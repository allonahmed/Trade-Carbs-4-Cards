...
export default class HomePage extends Component{
    ...
    render() {
        const GuestClick = () => { // for info purposes 
            return this.setState = (state => ({
                guest: state.guest = true,
            }))
        }

        const MouseOn = () => { // if Moused over, changes the state of the container class, to add styling to other elements when hovering over our buttons
            return this.setState(state=> ({
                containerClass: state.containerClass = 'home-container edit-background',
        ...
        return (
            <div class='container'> 
                <div className={this.state.containerClass} >
                    ...
                    <div className='icon-div'><i className="fa fa-sign-in-alt"></i></div>
                        <Link to = '/login' style= {{textDecoration: 'none'}}> <button className={this.state.buttonClass} onMouseOver={MouseOn} onMouseLeave={MouseLeave}> <p>Login to Carbs 4 Cards</p> <img className='home-button-img'src={Login} alt='button-logo'></img></button> </Link> 
                        <Link to = '/game' style= {{textDecoration: 'none'}}><button  onClick = {GuestClick} className={this.state.buttonClass} onMouseOver={MouseOn} onMouseLeave={MouseLeave}> <p style={{marginLeft:'7.5rem'}}>Play as a Guest</p><img className='home-button-img' src= {Guest} alt='guest-play'></img> </button></Link>
                    </div>
                </div>
            </div>
        )
